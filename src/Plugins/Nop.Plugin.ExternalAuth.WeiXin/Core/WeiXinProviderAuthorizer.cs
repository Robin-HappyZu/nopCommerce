...
namespace Nop.Plugin.ExternalAuth.WeiXin.Core
{
    public class WeiXinProviderAuthorizer : IOAuthProviderWeiXinAuthorizer
    {
        ...
        {
            get { return _weiXinApplication ?? (_weiXinApplication = new WeiXinClient(_weiXinExternalAuthSettings.ClientKeyIdentifier, _weiXinExternalAuthSettings.ClientSecret)); }
        }

        private AuthorizeState VerifyAuthentication(string returnUrl)
        {
            var authResult = this.WeiXinApplication.VerifyAuthentication(_httpContext, GenerateLocalCallbackUri());
        ...
        private void AppendQueryArgs(UriBuilder builder, IEnumerable<KeyValuePair<string, string>> args)
        {
            if (args != null && args.Any())
            {
                ...
                // 微信特定后缀
                builder2.Append("#wechat_redirect");
